...
const uploadedFile = require('./uploaded-file-5a58b950.js');
...
const BardFile = class {
    ...
    constructor(hostRef) {
        ...
        this.fileTarget = uploadedFile.html(`<input id="${this.fileTargetId}">`);
        this.hiddenTargetId = `hidden-target-${this.el.getAttribute("name")}`;
        this.hiddenTarget = uploadedFile.html(`<input id="${this.hiddenTargetId}">`);
    }
    componentWillLoad() {
    ...
    set value(val) {
        ...
        if (JSON.stringify(this.value) !== JSON.stringify(newValue)) { // this is insane. javascript is fucking garbage.
            this.files = newValue.map(signedId => Object.assign(new uploadedFile.UploadedFile(), {
                name: this.name,
    ...
    fileTargetChanged(event) {
        if (event.target !== this.fileTarget)
            return;
        this.files.push(...Array.from(this.fileTarget.files).map(file => Object.assign(new uploadedFile.UploadedFile(), {
            name: this.name,
            preview: this.preview,
    ...
    removeUploadedFile(event) {
        uploadedFile.arrayRemove(this.files, event.detail);
        this.files = this.files;
    }
    fireChangeEvent() {
    ...
    componentDidRender() {
        uploadedFile.morphdom(this.fileTarget, `
      ...
      >`);
        uploadedFile.morphdom(this.hiddenTarget, `
      ...
      >`);
        ...
        uploadedFile.morphdom(this.el, wrapper, { childrenOnly: true });
    }
    // Validations
    checkValidity() {
...
//# sourceMappingURL=bard-file.cjs.entry.js.map